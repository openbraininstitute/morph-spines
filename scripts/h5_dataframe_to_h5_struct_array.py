...
It replaces the existing /edges table (in pandas dataframe format) with an HDF5
table while leaving the rest of the groups and datasets untouched.

Usage:
    python h5_dataframe_to_h5_struct_array.py <input_filepath> <output_filepath>
"""
...
def dataframe_to_struct_array(df: pd.DataFrame) -> NDArray:
    """Convert the given pandas DataFrame to a numpy structured array.

    Data types are inferred from the contents of each DataFrame column.
    Fixed-length byte strings ('S{n}') are used for string columns
    Args:
        df: The dataframe to convert to a numpy structured array.

    Returns: A structured numpy array containing the information of the dataframe, which is
    suitable for storing writing as an HDF5 compound dataset.

    """
    if df.empty:
        return np.zeros(0)

    # Infer column types based on types of the first row of elements
    dtypes: list[tuple[str, Any]] = []
    for col in df.columns:
        col_types = set(type(row) for row in df[col])
        if len(col_types) != 1:
            raise TypeError(f"Column {col} contains values of different types: {col_types}")
        col_type = col_types.pop()
        if np.issubdtype(col_type, np.number):
            dtypes.append((col, col_type))
        elif col_type is str:
            # If the column holds strings, scan the whole column to get the longest string
            max_length = df[col].str.len().max()
            col_type_str = f"S{max_length}"
            dtypes.append((col, col_type_str))
        else:
            # To be decided: raise exception or accept with warning
            print(f"WARNING: skipped unsupported dtype for column {col}: {col_type}")
            # raise TypeError(f"Unsupported dtype for column {col}: {col_type}")

    # Create data array, serializing rows
    data = np.empty(len(df), dtype=dtypes)
    for col, col_type in dtypes:
        if isinstance(col_type, str) and col_type.startswith("S"):
            data[col] = df[col].fillna("").to_numpy(col_type)
        else:
            data[col] = df[col].to_numpy(col_type)

    return data


def struct_array_to_dataframe(filename: str, dataset_path: str) -> pd.DataFrame:
    """Convert a numpy structured array (from HDF5 compound dataset) into a pandas DataFrame.

    Args:
        filename: HDF5 filepath.
        dataset_path: Path where the structured array is stored.

    Returns: a pandas DataFrame containing the information of the structured array.

    """
    return morph_spine_loader.load_spine_table(filename, dataset_path)


def convert_file(input_file: str, output_file: str) -> list[str]:
    """Copy an HDF5 file into another one and replace all pandas dataframes with structured arrays.
    ...
    Args:
        ...
        output_file: New HDF5 file, dataframes replaced with structured arrays (under the same
        path).
    ...
    with h5py.File(input_file, "r") as f_in, h5py.File(output_file, "w") as f_out:
        for name, _item in f_in.items():
            if name != "edges":
                ...
                # that don't contain dataframes + convert the dataframes into HDF5 tables
                to_copy: list[str] = []
                ...
                # Replace the pandas dataframes with converted structured arrays in the output file
                # (under the same path)
                for df_path in pandas_groups:
                    ...
                    table_out = dataframe_to_struct_array(df_in)
                    edges_out.create_dataset(df_path, data=table_out)
...
def test_equal_dataframes(in_file: str, out_file: str, df_path: str) -> bool:
    ...
    Args:
        ...
        out_file: Second file with data stored as structured array.
    ...
    """
    in_spine_table = morph_spine_loader.load_spine_table(in_file, df_path)
    out_spine_table = struct_array_to_dataframe(out_file, df_path)
